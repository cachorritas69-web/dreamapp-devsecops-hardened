/**
 * Firebase Function to generate weekly sleep statistics
 * 
 * This function runs weekly (scheduled) to calculate aggregated sleep statistics
 * for each user based on their last 7 sleep records.
 * 
 * Collection: weekly_sleep_stats
 * 
 * @param event - Scheduled event (runs every Monday at 9:00 AM)
 */
...
export const generateWeeklyStats = onSchedule("0 9 * * 1", async (event) => {
    logger.info("Starting weekly sleep statistics generation");
    ...
    try {
        ...
        for (const userId of uniqueUsers) {
            try {
                await generateWeeklyStatsForUser(userId);
        ...
        logger.info("Weekly sleep statistics generation completed successfully");
        
    } catch (error) {
        logger.error("Failed to generate weekly sleep statistics", error);
        throw error;
    }
});
 ...
 */
async function generateWeeklyStatsForUser(userId: string): Promise<void> {
    ...
    logger.info(`Generating weekly stats for user ${userId} from ${weekStartDateStr} to ${weekEndDateStr}`);
    ...
    if (sleepDataSnapshot.empty) {
        logger.warn(`No sleep data found for user ${userId} in the last 7 days`);
        return;
    }
    ...
    logger.info(`Found ${sleepRecords.length} sleep records for user ${userId}`);
    ...
    logger.info(`Weekly stats saved for user ${userId} with ID: ${weeklyStatsId}`);
}
 ...
 * @param sleepRecords - Array of sleep records for the week
 * @param sourceNights - Array of document IDs used for calculation
 * @param weekStartDate - Start date of the week
 * @param weekEndDate - End date of the week
 * @returns WeeklySleepStats object
 */
function calculateWeeklyStats(
    ...
    // Note: dataQuality is added in the metadata but not in the original schema
...
/**
 * Calculate trend based on first half vs second half of the data
 */
...
/**
 * Calculate average time from an array of Date objects
 */
...
/**
 * Calculate time consistency (how consistent are the times)
 */
function calculateTimeConsistency(times: Date[]): number {
    ...
    // Assuming 60 minutes variation = 0 consistency, 0 minutes variation = 100 consistency
